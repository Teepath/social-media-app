...
const { validateSignupData, validateLoginData, reduceUserDetails } = require("../utils/validation");
...
exports.signup = (req, res)=>{
  ...
  const noImage='blank-profile.png';
...
exports.addUserDetails = (req, res) =>{
    ...
    db.doc(`/users/${req.user.handle}`).update(userDetails)
        .then(()=> {
            return res.json({message: 'Details addedd successfully'})
        })
        .catch((err)=>{
            console.error(err)
...
exports.markNoficationsRead = (req, res)=>{
    let batch = db.batch();
    req.body.forEach(notificationId =>{
        const notifcation = db.doc(`/notifications/${notificationId}`)
        batch.update(notification, {rea: true})
    });
    batch.commit()
    .then(()=>{
